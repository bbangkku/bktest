...
print("입장 기록 많은 Top3")
for i in range(3):
    print(f'{entry.most_common(3)[i][0]} {entry.most_common(3)[i][1]}회')
print('\n')
# 수상한 사람 멘트
print("출입 기록이 수상한 사람")
for keys in (entry-exit):
    print(f'{keys}은 입장 기록이 ',end='')
    for key in (entry-exit).values():
        print(f'{key}회 더 많아 수상합니다.')

for keys in (exit-entry):
    print(f'{keys}은 퇴장 기록이 ',end='')
    for key in (exit-entry).values():
        print(f'{key}회 더 많아 수상합니다.')
